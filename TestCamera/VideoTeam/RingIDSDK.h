...
void notifyClientMethodWithAudioAlarmIos(LongLong lFriendID, int data[], int dataLenth);
...
public:
    ...
    int StartMultiResolutionVideoFileSession(const char *outputFilePath, int *targetHeight, int *targetWidth, int iLen);
    int MakeMultiResolutionVideoFile( const char *inputFilePath, const char *fileName, int iLen );
    int StopMultiResolutionVideoFileSession();
    
    bool StartAudioCall(const LongLong& lFriendID, int nServiceType, int entityType, int nAudioSpeakerType);
    
    bool StartCallInLive(const LongLong& llFriendID, int iRole, int nCallInLiveType, int nTraceInfoLength, int * npTraceInfo, int nScreenSplitType);
    ...
    void PushPacketForDecodingVector(LongLong lFriendID, int mediaType, int nEntityType, unsigned char *in_data, int in_size, std::vector< std::pair<int, int> > missing);
    ...
    void SetNotifyClientWithAudioAlarmCallback(void(*callBackFunctionPointer)(LongLong, int*, int));
