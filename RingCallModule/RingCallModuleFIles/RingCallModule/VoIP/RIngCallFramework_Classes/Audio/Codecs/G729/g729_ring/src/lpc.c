 ...
 *-------------------------------------------------------------*/
 ...
 for (i = 0; i< NC; i++)
 {
   ...
   if (L_temp1 > 0x00007fff || L_temp1 < (Word32)0xffff8000)
     break;
   f1[i+1] = (Word16)L_temp1;

   /* f2[i+1] = a[i+1] - a[M-i] + f2[i] */
   L_temp2 = (Word32)y + (Word32)f2[i];
   if (L_temp2 > 0x00007fff || (L_temp2 < (Word32)0xffff8000))
     break;
...
static Word16 Chebps_11(Word16 x, Word16 f[], Word16 n)
{
 ...
 /* Note: All computation are done in Q24. */
  ...
  /* Result in Q14              */
  if ((UWord32)(t0 - 0xfe000000) < 0x01ffffff -  0xfe000000)
    cheb = (Word16)(t0 >> 10);
  else
    cheb = t0 > (Word32) 0x01ffffff ? MAX_16 : MIN_16;
...
static Word16 Chebps_10(Word16 x, Word16 f[], Word16 n)
{
 ...
 /* Note: All computation are done in Q23. */
  ...
  /* Result in Q14              */
  if ((UWord32)(t0 - 0xff000000) < 0x00ffffff -  0xff000000)
    cheb = (Word16)(t0 >> 9);
  else
    cheb = t0 > (Word32) 0x00ffffff ? MAX_16 : MIN_16;
